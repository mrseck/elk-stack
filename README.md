# The Elastic stack Project 100% made with docker container

Elastic stack Project 100% made with docker container, TLS secured, with Elasticsearch, Kibana, Fleet, and the Detection Engine all pre-configured, enabled and ready to use, within minutes.
...
4. Change the default password of `elasticpasswd` in the `.env` file (don't change the `elastic` username, it's a [required built-in user](https://www.elastic.co/guide/en/elasticsearch/reference/current/built-in-users.html))  
